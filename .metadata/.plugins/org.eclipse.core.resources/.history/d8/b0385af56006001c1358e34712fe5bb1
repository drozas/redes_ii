...
public class GeneradorHtml {
	 ...
	 * 
	 */
	public static StringBuilder generaListaTodos(String credencial, ConcurrentHashMap<String, Peer> peersConectados){
		Html page = new Html();
		
		page.setTile(Constantes.TERMINAL + "Ver peers conectados");
		page.addBr();
		HtmlP p = page.addP();
		p.addText("Hay " + peersConectados.size() + " peers conectados a este Terminal");
		p.setAlign(HtmlP.AlignType.CENTER);
		page.addHr();
		
	
		//Presentación de peers conectados con tabla
		if (peersConectados.size()>0)
		{
			
			int nFilas = peersConectados.size();
			HtmlTable table = page.addTable();
			...
			Enumeration<Peer> iterador = peersConectados.elements();
			int i = 0;
			while (iterador.hasMoreElements())
			{
				...
				td[i][0].addText(peer.getUsuario().getNombreCompleto());
				td[i][0].addText(" ("+ peer.getUsuario().getNick() + ")");
				td[i][0].addText(" en " + peer.getUsuario().getHost() + ":" + peer.getUsuario().getPuertoP2P());
				
				//Y los enlaces en 1 y 2				
				td[i][1].addA(Constantes.DIR_REMOTO + "?"+ Constantes.CREDENCIAL_VAR + "=" + credencial 
						+ "&" + Constantes.USUARIO_REMOTO_VAR + "=" + peer.getUsuario().getNick(),
				"[Listar ficheros]");
			...
			p.addText("No hay ningún usuario conectado");
			p.setAlign(HtmlP.AlignType.CENTER);
		}
